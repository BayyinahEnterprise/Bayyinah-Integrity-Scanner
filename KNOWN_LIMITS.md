...
- v2.0.0: §10 NAMING.md FileRouter calibration target filed (NAMING.md ~12 KB of markdown prose with comma-laden lines mis-routed by FileRouter's CSV content sniff, 126 false-positive CSV findings). Caught by the v2.0.0 recursive self-verification harness. Closed at v2.0.1 (Round 21) below.
- v2.0.1: §10 NAMING.md FileRouter calibration CLOSED (Round 21). The FileRouter's `_detect_csv` content-sniff branch in `infrastructure/file_router.py::FileRouter.detect` is now gated by an extension guard: when `ext_kind is FileKind.MARKDOWN`, the CSV branch is skipped and routing falls through to the markdown extension-based fall-through. Closure absorbed via the v2.0.0 structural defense: `TestPendingCalibrationDiagnostic` fired with `PROMOTE NAMING.md` when NAMING.md started scanning clean, forcing promotion to `_TOP_LEVEL_DOCS`. Known bounded trade-off (NEW limitation class at v2.0.1): a CSV deliberately renamed to .md to evade CSV-specific analyzers will not trigger the CSV content sniff; the markdown analyzer's own content-shape findings still apply. PARITY confirmed intact for all 8 Phase 0 .md fixtures (none currently triggered the CSV mis-route prior to calibration).
