...
[![Version](https://img.shields.io/badge/version-1.1.6-green.svg)](https://github.com/BayyinahEnterprise/Bayyinah-Integrity-Scanner/blob/main/CHANGELOG.md)
...
1,734 pytest cases across domain, infrastructure, analyzers, application, fixture corpora, and end-to-end integration (57 test files, verified on 1.1.6). Every Phase 0 fixture is asserted for exact mechanism firings, score, error text, and `scan_incomplete` flag, against both `bayyinah_v0.py` and `bayyinah_v0_1.py`. Byte-identical PDF parity with the reference implementation is re-verified after every phase; the invariant has held across every release from 0.2.x through 1.1.6.
