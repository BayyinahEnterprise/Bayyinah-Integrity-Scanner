#!/usr/bin/env python3
"""
Bayyinah v0.1 — PDF File Integrity Scanner (Modular Analyzer Architecture)
==========================================================================

Same detection behavior as bayyinah_v0.py. The refactor moves from a pair of
monolithic scanner classes (TextLayerScanner, ObjectLayerScanner) to a
uniform plugin contract:

    BaseAnalyzer    — abstract base; every analyzer implements
                      .scan(ctx: PDFContext) -> list[Finding]
    PDFContext      — shared, lazily-cached parsing state (fitz + pypdf)
    ScanService     — orchestrator; per-analyzer failure isolation;
                      applies scan_incomplete clamp at aggregation
    Finding         — unchanged; still carries surface/concealed for
                      inversion recovery and optional severity_override
    IntegrityReport — unchanged JSON schema; tool consumers untouched

v0.1 keeps the two existing analyzers as "fat" classes — all sub-detectors
remain methods. Per-mechanism splitting is deferred: the first correctness
goal is byte-identical output against the v0 regression corpus. Splitting
comes after parity is proven.

The monolithic v0 file (bayyinah_v0.py) stays in the repo as the reference
implementation until parity is verified across all corpora.

Operationalizes Section 9 (input-level performed alignment) of the
Munafiq Protocol (DOI: 10.5281/zenodo.19677111).

Usage:
    python bayyinah_v0_1.py <pdf_path>
    python bayyinah_v0_1.py <pdf_path> --json
    python bayyinah_v0_1.py <pdf_path> --quiet
...
MICROSCOPIC_FONT_THRESHOLD = 1.0
BACKGROUND_LUMINANCE_WHITE = 1.0
COLOR_CONTRAST_THRESHOLD = 0.05
...
# ---------------------------------------------------------------------------
# Dataclasses (unchanged from v0 — public JSON schema must not drift)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# PDFContext — shared parsing state, lazily built, analyzer-agnostic
# ---------------------------------------------------------------------------

class PDFContext:
    """Lazily-cached view over a single PDF, shared across analyzers.

    Analyzers read from this context instead of re-opening the file. The
    context owns the fitz document and (on demand) the pypdf reader, plus
    memoized per-page extractions that would otherwise be duplicated across
    analyzers.

    Analyzers MUST NOT mutate the context; it is a read-through cache.
    """
    ...
    @property
    def fitz(self):
        """The fitz (pymupdf) Document. Open failures surface to the caller."""
    ...
    def try_pypdf(self) -> tuple[Any | None, Exception | None]:
        """Return (pypdf.PdfReader, None) on success or (None, exception)
        on failure. Exception is captured on first attempt and replayed on
        subsequent calls — analyzers opt in to pypdf failure handling
        without the context raising through them."""
        ...
        try:
            self._pypdf_reader = pypdf.PdfReader(str(self.path))
            return self._pypdf_reader, None
    ...
    def raw_bytes(self) -> bytes | None:
        """Whole-file bytes, cached. Returns None if read fails."""
...
# ---------------------------------------------------------------------------
# BaseAnalyzer — the one abstraction the refactor turns on
# ---------------------------------------------------------------------------

class BaseAnalyzer(ABC):
    """Uniform contract for every analysis capability.

    Subclasses declare:
        name            — string identifier (used in logs and finding sources)
        error_prefix    — string used by ScanService when this analyzer raises

    Subclasses implement:
        scan(ctx)       — return list of findings. Analyzer-internal failures
                          that are expected (e.g. pypdf open failure in
                          ObjectLayerAnalyzer) SHOULD be converted into
                          scan_error findings inside the analyzer. Unexpected
                          failures MAY raise — ScanService catches them and
                          records the error at report level with this
    ...
    @abstractmethod
    def scan(self, ctx: PDFContext) -> list[Finding]:
        ...


# ---------------------------------------------------------------------------
# TextLayerAnalyzer — port of TextLayerScanner from v0
# ---------------------------------------------------------------------------

class TextLayerAnalyzer(BaseAnalyzer):
    ...
    def _scan_render_modes(self, page, page_idx: int) -> list[Finding]:
        ...
        for (start, end) in invisible_regions:
            ...
            if concealed:
                ...
                # Capability-only sub-variant (Akira OCR noise floor):
                # render-mode-3 armed but no text drawn. Down-tier.
                findings.append(Finding(
                    mechanism="invisible_render_mode",
                    tier=3,
                    confidence=0.6,
    ...
    def _extract_tj_strings(segment: str) -> str:
        ...
        for m in re.finditer(r"\((.*?)(?<!\\)\)\s*Tj", segment, re.DOTALL):
            pieces.append(TextLayerAnalyzer._decode_pdf_literal(m.group(1)))
        for m in re.finditer(r"\[(.*?)\]\s*TJ", segment, re.DOTALL):
            arr = m.group(1)
            for lit in re.finditer(r"\((.*?)(?<!\\)\)", arr, re.DOTALL):
                pieces.append(TextLayerAnalyzer._decode_pdf_literal(lit.group(1)))
        return "".join(pieces)
    ...
    def _scan_raw_unicode(
        ...
        # /ActualText hex (UTF-16BE per spec)
        for m in re.finditer(
    ...
    def _check_unicode(text, bbox, page_idx) -> list[Finding]:
        ...
        findings.extend(TextLayerAnalyzer._check_homoglyphs(text, bbox, page_idx))
        return findings
...
# ---------------------------------------------------------------------------
# ObjectLayerAnalyzer — port of ObjectLayerScanner from v0
# ---------------------------------------------------------------------------

class ObjectLayerAnalyzer(BaseAnalyzer):
    ...
    def scan(self, ctx: PDFContext) -> list[Finding]:
        findings: list[Finding] = []
        findings.extend(self._scan_incremental_updates(ctx))

        reader, err = ctx.try_pypdf()
        if err is not None:
            findings.append(Finding(
    ...
    def _scan_incremental_updates(self, ctx: PDFContext) -> list[Finding]:
        data = ctx.raw_bytes()
        if data is None:
            return []
...
# ---------------------------------------------------------------------------
# Analyzer registry (default dispatch order)
# ---------------------------------------------------------------------------

DEFAULT_PDF_ANALYZERS: list[type[BaseAnalyzer]] = [
    TextLayerAnalyzer,
    ObjectLayerAnalyzer,
]
...
# ---------------------------------------------------------------------------
# Scoring + summary (verbatim from v0)
# ---------------------------------------------------------------------------
...
class ScanService:
    """Build a PDFContext once, dispatch every analyzer, aggregate findings.

    Per-analyzer exceptions are caught and translated into report.error using
    the analyzer's error_prefix. This preserves v0's error message format
    ("Text layer scan error: X; Object layer scan error: Y") so downstream
    consumers of report.error keep working byte-for-byte.

    Expected-failure cases (e.g. pypdf open failure inside ObjectLayerAnalyzer)
    are the analyzer's responsibility — it converts them into scan_error
    findings. The scan_incomplete clamp here catches both routes.
    """

    def __init__(self, analyzers: list[BaseAnalyzer] | None = None):
        if analyzers is None:
            analyzers = [cls() for cls in DEFAULT_PDF_ANALYZERS]
    ...
    def scan(self, pdf_path: Path) -> IntegrityReport:
        ...
        try:
            ...
            _ = ctx.fitz  # force open; fail early if pymupdf can't parse
        except Exception as e:
        ...
        try:
            for analyzer in self.analyzers:
                try:
                    report.findings.extend(analyzer.scan(ctx))
                except Exception as e:
                    msg = f"{analyzer.error_prefix}: {e}"
                    report.error = (
                        f"{report.error}; {msg}" if report.error else msg
                    )
        finally:
            ctx.close()
        ...
        # v0.1 scan_incomplete clamp — invariant with v0.
...
# ---------------------------------------------------------------------------
# Public entry point — preserved signature from v0
# ---------------------------------------------------------------------------

def scan_pdf(pdf_path: Path) -> IntegrityReport:
    """Public entry point — preserved signature and semantics from v0."""
    return ScanService().scan(pdf_path)
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="bayyinah_v0_1",
        description=(
            "Bayyinah v0.1 — PDF file integrity scanner (modular analyzer "
            "architecture). Detects concealment mechanisms and reports the "
            "gap between what the file displays and what the file contains."
        ),
    )
