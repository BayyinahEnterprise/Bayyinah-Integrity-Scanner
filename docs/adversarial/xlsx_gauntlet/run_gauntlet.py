...
def scan_local(path: Path) -> dict:
    ...
    pdf_gauntlet/run_gauntlet_local.py and docx_gauntlet/
    run_gauntlet.py). Fall back to the manual builder for older
    report shapes.
    """
...
def run(mode: str) -> list[dict]:
    fixtures = sorted(
        f for f in os.listdir(FIXTURES_DIR) if f.endswith(".xlsx")
    )
    results = []
