...
Measures sequential scan time on a four-density PDF panel with P50 over
5 measured runs (preceded by a single warm-up run discarded).
...
Outputs Markdown report at docs/benchmarks/v1_1_4_four_density.md.
...
def main() -> None:
    ...
    # Write report.
    lines = [
        f"# v{bayyinah.__version__} four-density benchmark",
        "",
        "## Methodology",
        "",
    ...
    out = _REPO_ROOT / "docs/benchmarks/v1_1_4_four_density.md"
    out.write_text("\n".join(lines))
    print(f"\nwrote {out}")
