...
def test_registry_count_is_exact_108() -> None:
    ...
    assert len(MECHANISM_REGISTRY) == 108, (
        f"Mechanism count drift: expected 108 (27 zahir + 81 batin), "
...
def test_registry_is_union_of_zahir_and_batin() -> None:
    assert MECHANISM_REGISTRY == ZAHIR_MECHANISMS | BATIN_MECHANISMS


def test_zahir_and_batin_are_disjoint() -> None:
    """Every mechanism belongs to exactly one source layer."""
    overlap = ZAHIR_MECHANISMS & BATIN_MECHANISMS
    assert overlap == set(), (
        f"Source-layer disjointness broken — these mechanisms are in "
        f"both ZAHIR and BATIN: {sorted(overlap)}"
    )
...
def test_every_tier_value_is_one_two_or_three() -> None:
    for mech, tier in TIER.items():
        assert tier in (1, 2, 3), (
            f"Tier out of {{1,2,3}} for {mech!r}: {tier}"
        )
