"""
Phase 18 fixture-level guardrails — every PPTX fixture fires exactly
its intended mechanism(s) through the full ``application.ScanService``
pipeline.

Structurally mirrors ``tests/test_xlsx_fixtures.py`` (XLSX corpus) and
``tests/test_docx_fixtures.py`` (DOCX corpus). The expectation table
``PPTX_FIXTURE_EXPECTATIONS`` lives next to the fixture builder in
``tests/make_pptx_fixtures.py`` and is the single source of truth for
...
PPTX fixtures are Office Open XML ZIP archives. They regenerate
byte-deterministically from ``tests/make_pptx_fixtures.py`` (fixed
ZipInfo.date_time, STORED compression) — same discipline as the XLSX
corpus in Phase 17 and the DOCX corpus in Phase 15.

Al-Baqarah 2:14: "When they meet those who believe, they say, 'We
believe,' but when they are alone with their devils, they say,
'Indeed, we are with you.'" A presentation is the format that most
literally ships different content to different audiences — the room
watches the slides, the presenter reads the notes, the ingestion
pipeline reads both. The fixtures here are the witnesses for every
surface where those three readings can diverge.
"""
...
@pytest.mark.parametrize(
    "rel,expected",
    sorted(PPTX_FIXTURE_EXPECTATIONS.items()),
)
...
def test_clean_fixtures_score_1_and_complete(scanner: ScanService) -> None:
    """Every clean PPTX fixture must score exactly 1.0 and be a complete scan."""
    for rel, expected in PPTX_FIXTURE_EXPECTATIONS.items():
        if expected:
            continue
...
def test_every_fixture_is_not_scan_incomplete(scanner: ScanService) -> None:
    """No fixture in the PPTX corpus should trigger a scan-incomplete clamp.

    Every PPTX fixture is a fully-parseable ZIP with well-formed
    PresentationML parts. If any of them trips ``scan_incomplete``, the
