"""
Phase 24 fixture-level guardrails — every video-format fixture fires
exactly its intended mechanism(s) through the full
``application.ScanService`` pipeline.

Structurally mirrors ``tests/test_image_fixtures.py`` (Phase 10) and
``tests/test_text_fixtures.py`` (Phase 9). The expectation table lives
in ``tests/make_video_fixtures.py`` — the single source of truth for
...
Two guarantees:
    ...
    * Clean fixtures fire zero non-``scan_error`` mechanisms *except*
      the always-on ``video_stream_inventory`` (non-deducting,
...
mechanism by design — and cross_stem_divergence.mp4 is listed without
...
FIXTURES_DIR: Path = (
    Path(__file__).resolve().parent / "fixtures" / "video"
)


# ---------------------------------------------------------------------------
# Autouse fixture regeneration
# ---------------------------------------------------------------------------
...
def _ensure_video_fixtures_built() -> None:
    ...
    Deterministic generators → byte-identical no-op when all files
    already exist. Only invoked when something is missing, to keep the
    test-run footprint small.
    """
    missing = [
        FIXTURES_DIR / rel
    ...
    try:
        ...
        subprocess.run(
            [sys.executable, "-m", "tests.make_video_fixtures"],
            check=True,
...
@pytest.mark.parametrize(
    ...
    sorted(VIDEO_FIXTURE_EXPECTATIONS.items()),
)
def test_fixture_fires_expected_mechanisms(
    ...
    We scan through ``ScanService`` (not ``VideoAnalyzer`` directly) so
    the FileRouter dispatch path is covered. ``scan_error`` is
    tolerated — malformed-input branches are tested independently in
    ``tests/analyzers/test_video_analyzer.py``.
    """
...
# ---------------------------------------------------------------------------
# Clean-fixture invariants: score 1.0, not incomplete, no error
# ---------------------------------------------------------------------------


def _clean_fixture_paths() -> list[str]:
    return sorted(
        rel for rel, mechs in VIDEO_FIXTURE_EXPECTATIONS.items()
        if mechs == {"video_stream_inventory"}
    )
...
def test_clean_fixture_scores_one(rel_path: str, scanner: ScanService) -> None:
    """Clean and NULL-path fixtures score exactly 1.0.

    The expectation table distinguishes "clean" (only inventory fires)
    from "adversarial" (inventory + at least one deducting mechanism).
    Because ``video_stream_inventory`` has severity 0.0, any fixture
    whose expectation is only-inventory must produce score 1.0.
    """
...
def test_clean_fixture_scan_incomplete_is_false(
    ...
    assert report.scan_incomplete is False, (
        f"{rel_path}: clean fixture should not mark scan_incomplete"
    )
...
def test_clean_fixture_has_no_error(rel_path: str, scanner: ScanService) -> None:
    report = scanner.scan(FIXTURES_DIR / rel_path)
    # Clean scans do not set .error. A scan_error finding without the
    # top-level error string would also pass this (it's the per-analyzer
    # degradation path), but for a synthesised clean MP4 neither should
    # ever happen.
    assert not report.error, f"{rel_path}: unexpected error {report.error!r}"


# ---------------------------------------------------------------------------
# Adversarial-fixture invariants: score < 1.0 when the expectation set
# carries a deducting mechanism.
# ---------------------------------------------------------------------------
...
def test_adversarial_fixture_scores_below_one(
    ...
    """Every adversarial fixture must deduct — otherwise the mechanism
    it proves is unreachable from the pipeline."""
    report = scanner.scan(FIXTURES_DIR / rel_path)
    assert report.integrity_score < 1.0, (
        f"{rel_path}: expected score < 1.0, got {report.integrity_score}. "
